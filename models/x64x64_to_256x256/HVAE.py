...
class ConditionalHierarchicalEncoder(nn.Module):
    def __init__(self, in_channels, channels, latent_dims):
        super().__init__()
        self.in_conv = nn.Conv2d(in_channels, channels[0], 1, 1, 0)  # 256 -> 256
        self.down1 = nn.Conv2d(channels[0], channels[1], 4, stride=2, padding=1)  # 128
        self.down2 = nn.Conv2d(channels[1], channels[2], 4, stride=2, padding=1)  # 64 
        self.down3 = nn.Conv2d(channels[2], channels[3], 4, stride=2, padding=1)  # 32 
        
        self.down4 = nn.Conv2d(channels[3], channels[4], 4, stride=2, padding=1)  # 16x16
        self.down5 = nn.Conv2d(channels[4], channels[5], 4, stride=2, padding=1)  # 8x8

        self.fc_mu1 = nn.Linear(channels[5] * 8*8, latent_dims[0])
        self.fc_logvar1 = nn.Linear(channels[5] * 8*8, latent_dims[0])

        self.fc_mu2 = nn.Linear(channels[4] * 16*16, latent_dims[1])
        self.fc_logvar2 = nn.Linear(channels[4] * 16*16, latent_dims[1])

        self.fc_mu3 = nn.Linear(channels[3] * 32*32, latent_dims[2])
        self.fc_logvar3 = nn.Linear(channels[3] * 32*32, latent_dims[2])
    ...
    def forward(self, x):
        ...
        #print(x5.shape)
        mu1, logvar1 = self.fc_mu1(x5.view(x5.size(0), -1)), self.fc_logvar1(x5.view(x5.size(0), -1))
        mu2, logvar2 = self.fc_mu2(x4.view(x4.size(0), -1)), self.fc_logvar2(x4.view(x4.size(0), -1))
        mu3, logvar3 = self.fc_mu3(x3.view(x3.size(0), -1)), self.fc_logvar3(x3.view(x3.size(0), -1))
...
class ConditionalHierarchicalDecoder(nn.Module):
    def __init__(self, out_channels, channels, latent_dims, condition_dims):
        super().__init__()
        self.channels = channels
        self.fc1 = nn.Linear(latent_dims[0], channels[5] *8*8)
        self.fc2 = nn.Linear(latent_dims[1], channels[4] * 16*16)
        self.fc3 = nn.Linear(latent_dims[2], channels[3] * 32*32)

        self.fc_cond1 = nn.Linear(condition_dims[0], channels[5] * 8*8)
        self.fc_cond2 = nn.Linear(condition_dims[1], channels[4] * 16*16)
        
        self.deconv1 = nn.ConvTranspose2d(channels[5] * 2, channels[4], 4, stride=2, padding=1)  # 4x4 -> 8x8
        self.deconv2 = nn.ConvTranspose2d(channels[4] * 2, channels[3], 4, stride=2, padding=1)  # 8x8 -> 16x16
        self.deconv3 = nn.ConvTranspose2d(channels[3], channels[2], 4, stride=2, padding=1)  # 16x16 -> 32x32

        self.deconv4 = nn.ConvTranspose2d(channels[2], channels[1], 4, stride=2, padding=1)
        self.deconv5 = nn.ConvTranspose2d(channels[1], channels[0], 4, stride=2, padding=1)
    ...
    def forward(self, z_hierarchical, condition):
        ...
        z1 = self.fc1(z1).view(-1, self.channels[5], 8, 8)
        z2 = self.fc2(z2).view(-1, self.channels[4], 16, 16)
        z3 = self.fc3(z3).view(-1, self.channels[3], 32, 32)

        c1 = self.fc_cond1(c1).view(-1, self.channels[5], 8, 8)
        c2 = self.fc_cond2(c2).view(-1, self.channels[4], 16, 16)
